...
from .finnhub_client import FinnhubClient, FINNHUB_TIMEFRAME_MAP
...
async def fetch_candles(client: FinnhubClient, symbol: str, timeframe: str, bars: int = 400) -> pd.DataFrame:
    now = int(datetime.now(tz=timezone.utc).timestamp())
    sec = _timeframe_to_seconds(timeframe)
    start = now - sec * (bars + 5)

    resolution = FINNHUB_TIMEFRAME_MAP.get(timeframe, timeframe)
    df = await client.get_forex_candles(symbol, resolution, start, now)
    if timeframe == "4h" and resolution == "60":
        df = FinnhubClient.resample_to_4h(df)
    return df
...
async def process_pair(cfg: AppConfig, cache: Cache, notifier: TelegramNotifier, client: FinnhubClient, symbol: str, timeframe: str):
    try:
...
async def run_agent(args):
    ...
    client = FinnhubClient(cfg.finnhub.api_key, cfg.finnhub.base_url)
